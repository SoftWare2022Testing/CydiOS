...
FRAMEWORK_HEADERS = '/Users/gjy/Desktop/Project/Experiments/Darjeeling/LibHola/resources/framework_headers'
FRAMEWORK_PKL = '/Users/tomwu/Desktop/comeOnAgain/frameworks.pkl'
OUTPUT_PLIST = '/Users/tomwu/Desktop/comeOnAgain/BitPrice_1.2.4.plist'
ERROR = dict()
memptr = []
miss_arg = dict()
blr = []
...
USELESS_TYPES = ['id', 'void', 'void *', 'void **', 'struct objc_object *', '__int64', 'char *', '_QWORD *', '_QWORD',
                 'const char *']
...
class CG:
    ...
    def add_edge(self, src, target, ea, _type, details=None, message=None, ccall=None, block=None):
        ...
        if ea not in self.invoke_info:
            self.invoke_info[ea] = []
        self.invoke_info[ea].append(message or ccall or block)
        if type(target) is long:
            for node in [src, target]:
                if node not in self.graph.nodes:
                    if EXPLORE_MODE_ON:
                        color = 'yellow'
                        exploration.add(target)
                    else:
                        color = 'black'
...
class Frameworks:
    ...
    def load():
        ...
        if not os.path.exists(pkl):
            for f in os.listdir(FRAMEWORK_HEADERS):
                Frameworks.build_from_file(os.path.join(FRAMEWORK_HEADERS, f))
...
class MachO:
    ...
    def build():
        ...
        for seg_name in ['__objc_classrefs', '__objc_superrefs']:
            seg = ida_segment.get_segm_by_name(seg_name)
            if seg:
                for ea in range(seg.startEA, seg.endEA, 8):
                    # clsRef = idc.get_name(ea, idc.ida_name.GN_VISIBLE)
                    clsRef = idc.Name(idc.Qword(ea))
                    MachO.classrefs[clsRef] = ea

        seg = ida_segment.get_segm_by_name('__objc_selrefs')
        if seg:
            for ea in range(seg.startEA, seg.endEA, 8):
                # selRef = idc.get_name(ea, idc.ida_name.GN_VISIBLE)
                selRef = idc.get_bytes(idc.Qword(ea), idc.get_item_size(idc.Qword(ea)) - 1)
                MachO.selrefs[selRef] = ea
...
class Message(CCall):
    ...
    def __init__(self, f, callsite, call, rec=None, sel=None, args=[]):
        CCall.__init__(self, f, callsite, call, args)
...
class TaskManager:
    ...
    def init_pool(self):
        ...
        # init method
        if 'init' in MachO.funcs_index_by_sel:
...
def locate_caller(f):
    ctx = set()
    if f in MachO.oc_methods:
        rec, sel = idc.GetFunctionName(f)[1:].strip('[]').split()
        if sel in MachO.selrefs:
            for xref in XrefsTo(MachO.selrefs[sel]):
                if idc.SegName(xref.frm) == '__text':
                    ctx.add(idc.get_func_attr(xref.frm, idc.FUNCATTR_START))
    elif f in MachO.subroutines:
        for xref in XrefsTo(f):
            if idc.SegName(xref.frm) == '__text':
                ctx.add(idc.get_func_attr(xref.frm, idc.FUNCATTR_START))
    return ctx

def find_callstack(f):
    finished = []
    current = [[f]]
    while current:
        new_current = []
        for stack in current:
            upper_callers = locate_caller(stack[-1])
            valid_callers = upper_callers - set(stack)  # avoid loop
            if not valid_callers:
                finished.append(stack)
            else:
                for caller in valid_callers:
                    new_stack = copy.deepcopy(stack)
                    new_stack.append(caller)
                    new_current.append(new_stack)
        current = new_current
    print(finished)


def dump_graph(g, root, name, clear=False):
    dot = "{}.dot".format(os.path.join(root, name))
    svg = "{}.svg".format(os.path.join(root, name))
    nx.drawing.nx_agraph.write_dot(g, dot)
    subprocess.check_output(['dot', '-Tsvg', dot, '-o', svg])
    if clear:
...
class Transition:
    ...
    def __init__(self, src, des, api, context):
        self.src = src
        self.des = des
        self.api = api
    ...
    def pprint(self):
        print('{}: {} -> {}'.format(self.callback, self.src, self.des))

class Callback:

    def __init__(self, register, target, action, event, handlers):
    ...
    def pprint(self):
        print('[{} {}]({})'.format(self.target, self.action, self.handlers))
...
# find transitions
TRANSITION_API = ['presentViewController:animated:', 'performSegueWithIdentifier:sender:',
                'presentViewController:animated:completion:', 'pushViewController:animated:']
transitions = []

callers = set()

for api_sel in TRANSITION_API:
    if api_sel in MachO.selrefs:
        for xref in idautils.XrefsTo(MachO.selrefs[api_sel]):
            print("-----Xrefs:")
            print(xref.type, XrefTypeName(xref.type),'from', hex(xref.frm), 'to', hex(xref.to))
            if idc.SegName(xref.frm) == '__text':
                callers.add(idc.get_func_attr(xref.frm, idc.FUNCATTR_START))
for f in callers:
    Func.analyze(f)

for callsite in CG.sharedCG.invoke_info:
    for call in CG.sharedCG.invoke_info[callsite]:
        if isinstance(call, Message):
            if type(call.sel) is str and call.sel in TRANSITION_API:
                print("-----Tests:")
                print(call.sel)
                try:
                    info = []
                    for var in [call.rec] + call.args:
                        if isinstance(var, ida_hexrays.lvar_t):
                            info.append(Utils.nice_str(var.type().__str__()))
                        else:
                            info.append(var)
                    src, des = info[0], info[1]
                    print("src: ", src)
                    print("des: ", des)
                    if call.sel == 'pushViewController:animated:':
                        src = None
                    transitions.append(Transition(src, des, call.sel, call.ctx))
                except Exception as e:
                    print(e)
...
callbacks = []
registers = set()
register_call = 'addTarget:action:forControlEvents:'
if register_call in MachO.selrefs:
    for xref in idautils.XrefsTo(MachO.selrefs[register_call]):
        if idc.SegName(xref.frm) == '__text':
            registers.add(idc.get_func_attr(xref.frm, idc.FUNCATTR_START))
for f in registers:
    Func.analyze(f)
for callsite in CG.sharedCG.invoke_info:
    for call in CG.sharedCG.invoke_info[callsite]:
        if isinstance(call, Message):
            if type(call.sel) is str and call.sel == register_call:  # 查看还有什么其他可能
                try:
                    info = []
                    for var in [call.rec] + call.args:
                        if isinstance(var, ida_hexrays.lvar_t):
                            info.append(var.type().__str__())
                        else:
                            info.append(var)
                    handler = set()
                    if info[2] in MachO.funcs_index_by_sel:
                        if info[1] in MachO.funcs_index_by_sel[info[2]]:
                            handler.add(MachO.funcs_index_by_sel[info[2]][info[1]])
                        else:
                            handler = set(MachO.funcs_index_by_sel[info[2]].values())
                    callbacks.append(Callback(info[0], info[1], info[2], info[3], handler))
                except Exception as e:
                    print(e)
for sel in ['tableView:didSelectRowAtIndexPath:', 'collectionView:didSelectItemAtIndexPath:']:
    if sel in MachO.funcs_index_by_sel:
        for rec, f in MachO.funcs_index_by_sel[sel].items():
            callbacks.append(Callback(None, rec, sel, None, {f}))

# Find the callback that caused the transition
callback_funcs = set()
for cb in callbacks:
    callback_funcs.update(cb.handlers)
for f in callback_funcs:
    if f not in CG.sharedCG.graph.nodes:
        CG.sharedCG.graph.add_node(f)
    CG.sharedCG.graph.nodes[f]['color'] = 'blue'

for tran in transitions:
    f = tran.context
    if f in CG.sharedCG.graph.nodes:
        if CG.sharedCG.graph.nodes[f]['color'] == 'blue':
            CG.sharedCG.graph.nodes[f]['color'] = 'green'
        else:
            CG.sharedCG.graph.nodes[f]['color'] = 'red'
    else:
        CG.sharedCG.graph.add_node(f, color='red')
    if tran.context in callback_funcs:
        tran.callback = idc.GetFunctionName(tran.context)
        if tran.src is None:
            tran.src = Utils.nice_str(MachO.func_data[tran.context][0].type.__str__())
    else:
        pass  # check callstack
    if tran.src is None:
        pass  # check callstack
...
exploration = copy.deepcopy(callback_funcs)
while exploration-set(Func.pool):
    for f in list(exploration):
        Func.analyze(f)


trans_node_indexed_by_ea = dict()
for t in transitions:
    if t.context not in trans_node_indexed_by_ea:
        trans_node_indexed_by_ea[t.context] = []
    trans_node_indexed_by_ea[t.context].append(t)

useful_callbacks = dict()
for node in callback_funcs:
    trans = set()
    if node in CG.sharedCG.graph:
        for _node in list(nx.algorithms.dag.descendants(CG.sharedCG.graph, node))+[node]:
            if _node in trans_node_indexed_by_ea:
                trans.add(_node)
    if trans:
        d = dict()
        for ea in trans:
            sels = list(set([t.api for t in trans_node_indexed_by_ea[ea]]))
            d[idc.GetFunctionName(ea)] = sels
        useful_callbacks[idc.GetFunctionName(node)] = d
plistlib.writePlist(useful_callbacks, OUTPUT_PLIST)
